...
async function pgOrders(c){
  ...
  // Đếm badge trạng thái (toàn bộ, không phân trang)
  ...
  // Phân trang — reset về trang 1 khi filter/search thay đổi
  ...
  const list=filtered.slice(pageStart,pageStart+ORDER_PAGE_SIZE);
  ...
  const pagBtns=()=>{
    ...
    let btns='';
    // Prev
    btns+=`<button class="btn btn-sm${ORDER_PAGE===1?' disabled':''}" style="min-width:32px" onclick="if(ORDER_PAGE>1){ORDER_PAGE--;renderOrderTable()}" ${ORDER_PAGE===1?'disabled':''}><i class="ti ti-chevron-left"></i></button>`;
    // Số trang — hiện tối đa 5 nút
    const start=Math.max(1,Math.min(ORDER_PAGE-2,totalPages-4));
    const end=Math.min(totalPages,start+4);
    for(let i=start;i<=end;i++){
      btns+=`<button class="btn btn-sm${i===ORDER_PAGE?' btn-primary':''}" style="min-width:32px" onclick="ORDER_PAGE=${i};renderOrderTable()">${i}</button>`;
    }
    // Next
    btns+=`<button class="btn btn-sm${ORDER_PAGE===totalPages?' disabled':''}" style="min-width:32px" onclick="if(ORDER_PAGE<${totalPages}){ORDER_PAGE++;renderOrderTable()}" ${ORDER_PAGE===totalPages?'disabled':''}><i class="ti ti-chevron-right"></i></button>`;
    return`<div style="display:flex;gap:4px;align-items:center">${btns}</div>`;
  };
  ...
  <div class="toolbar">
    ...
    <span class="ml-auto" style="font-size:11.5px;color:var(--text-muted)">${filtered.length} vận đơn · trang ${ORDER_PAGE}/${totalPages}</span>
  </div>
  <div class="tbl-wrap"><table class="tbl" id="orders-table">
    <colgroup>
    ...
    ${renderOrderRows(list,canM)}
    </tbody>
  </table></div>
  <div style="display:flex;justify-content:space-between;align-items:center;padding:10px 4px 4px;flex-wrap:wrap;gap:8px">
    <span style="font-size:11.5px;color:var(--text-muted)">Hiển thị ${pageStart+1}–${Math.min(pageStart+ORDER_PAGE_SIZE,filtered.length)} / ${filtered.length} đơn</span>
    ${pagBtns()}
  </div>`;

  // Lưu filtered để renderOrderTable dùng (không fetch lại DB)
  window._filteredOrders=filtered;
  window._canM=canM;
}

// Render chỉ phần tbody — dùng khi chuyển trang (không fetch DB lại)
function renderOrderRows(list,canM){
...
// Chuyển trang KHÔNG fetch DB — chỉ re-slice filtered đã có sẵn
function renderOrderTable(){
  const filtered=window._filteredOrders||[];
  const canM=window._canM||false;
  const totalPages=Math.max(1,Math.ceil(filtered.length/ORDER_PAGE_SIZE));
  if(ORDER_PAGE>totalPages)ORDER_PAGE=totalPages;
  if(ORDER_PAGE<1)ORDER_PAGE=1;
  const pageStart=(ORDER_PAGE-1)*ORDER_PAGE_SIZE;
  ...
  // Chỉ update tbody + pagination info — không re-render toàn bộ
  const tbody=document.getElementById('orders-tbody');
  if(tbody)tbody.innerHTML=renderOrderRows(list,canM);

  // Update pagination buttons
  const pag=document.querySelector('#content > div:last-child');
  if(pag){
    let btns='';
    btns+=`<button class="btn btn-sm${ORDER_PAGE===1?' disabled':''}" style="min-width:32px" onclick="if(ORDER_PAGE>1){ORDER_PAGE--;renderOrderTable()}" ${ORDER_PAGE===1?'disabled':''}><i class="ti ti-chevron-left"></i></button>`;
    const start=Math.max(1,Math.min(ORDER_PAGE-2,totalPages-4));
    ...
    btns+=`<button class="btn btn-sm${ORDER_PAGE===totalPages?' disabled':''}" style="min-width:32px" onclick="if(ORDER_PAGE<${totalPages}){ORDER_PAGE++;renderOrderTable()}" ${ORDER_PAGE===totalPages?'disabled':''}><i class="ti ti-chevron-right"></i></button>`;
    pag.innerHTML=`<span style="font-size:11.5px;color:var(--text-muted)">Hiển thị ${pageStart+1}–${Math.min(pageStart+ORDER_PAGE_SIZE,filtered.length)} / ${filtered.length} đơn</span><div style="display:flex;gap:4px;align-items:center">${btns}</div>`;
  }
}
...
async function refreshOrder(id){
  // 1. Fetch full data (*) trước — pgOrders chỉ fetch COLS giới hạn sẽ overwrite ORDERS
  const{data}=await db.from('van_don').select('*').eq('id',id).single();
  // 2. Refresh danh sách — PHẢI await để hoàn tất trước bước 3
  await pgOrders(document.getElementById('content'));
  // 3. Patch lại ORDERS bằng full data, rồi re-render detail panel
  if(data){
